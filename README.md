...
## 配套工具：GUI 皮肤热切换器（skin-switcher）

`skin-switcher/` 是一个非皮肤插件，给 GUI 加一个右上角"换肤"按钮，在原皮 / 女仆工坊 / 虎鲸链路 之间点选热切换（写两个 patch 层 → watcher 热重放 → 自动刷新页面，全程不重启）：

```sh
dsh plugin --profile web add <本仓库路径>/skin-switcher
# 重启 dsh web 一次完成注册，之后切换全部热生效
```

详见 [skin-switcher/README.md](skin-switcher/README.md)。
